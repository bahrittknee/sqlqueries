SELECT click.click_id,
  ...
  FROM api_click_export click
    JOIN LATERAL ( SELECT conv.conversion_v3_id AS conversion_id,
         ...
         WHERE conv.request_date > click.request_date AND conv.visitor_id = click.visitor_id AND conv.conversion_event_type_id = 1
         ORDER BY conv.request_date
        LIMIT 3) conversions ON true
 WHERE (click.visitor_id IN ( SELECT api_conversion_export.visitor_id AS tracking_log_v3_id
